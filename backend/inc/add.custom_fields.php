...
if(isset($_GET['new_custom_page'])) {
	...
	$sql = "ALTER TABLE se_pages ADD $new_custom_page LONGTEXT";
	...
	$sql = "ALTER TABLE se_pages_cache ADD $new_custom_page LONGTEXT";
	if($db_mysql->query($sql)) {
		echo '<p>ALTER TABLE DONE '.$new_custom_page.'</p>';
	} else {
		var_dump( $db_mysql->error() );
...
if(count($custom_content) > 0) {
	foreach($custom_content as $new) {
		...
		$query = "SELECT count(*) FROM information_schema.COLUMNS WHERE (TABLE_SCHEMA = '$database_name') AND (TABLE_NAME = 'se_pages') AND (COLUMN_NAME = '$new')";
		$cnt_tables = $db_mysql->query($query)->fetch();		
		if($cnt_tables[0]>0) {
			echo '<p class="text-success">Column '.$new.' exists in fc_pages ...</p>';	
		} else {
			echo '<p>ADD <a href="?tn=addons&sub=migrate.mod&a=change_de&new_custom_page='.$new.'">'. $new.'</a> to se_pages table</p>';
		}
...
if(count($custom_user) > 0) {
	foreach($custom_user as $new) {
		...
		$query = "SELECT count(*) FROM information_schema.COLUMNS WHERE (TABLE_SCHEMA = '$database_name') AND (TABLE_NAME = 'se_user') AND (COLUMN_NAME = '$new')";
		$cnt_tables = $db_mysql->query($query)->fetch();		
		if($cnt_tables[0]>0) {
			echo '<p class="text-success">Column '.$new.' exists in fc_user ...</p>';	
		} else {
			echo '<p>ADD <a href="?tn=addons&sub=migrate.mod&a=change_db&new_custom_user='.$new.'">'. $new.'</a> to se_user table</p><hr>';
		}
	}
} else {
