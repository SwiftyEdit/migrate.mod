...
define('SE_PREFIX', $mig_prefs['prefs_database_prefix']);
...
$protocol =  '<h2>Found '.$cnt_all_tables.' Table files to create Tables</h2>';

for($i=0;$i<$cnt_all_tables;$i++) {
	
	unset($db_path,$table_name,$table_type,$database,$cols,$sql,$cnt_tables);
	include_once $all_tables[$i];
	...
	if($cnt_tables[0] < 1) {
		if($table_type == 'virtual' OR $database == 'index') {
            ...
            $protocol .= 'New Table: '.$all_tables[$i].'<hr>';
			//$protocol .= 'SQL for: '.print_r($sql,true);
			
			$db_mysql->query($sql);
            /*
            echo 'TBLE :'.$all_tables[$i].'<hr>';
			echo '<pre>';
