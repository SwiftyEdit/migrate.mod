...
if(!is_file("$mod_db")) {
	...
	$sql_entries_table = generate_sql_query(SE_CONTENT."/modules/migrate.mod/install/tpl-entries.php");
	$sql_prefs_table = generate_sql_query(SE_CONTENT."/modules/migrate.mod/install/tpl-preferences.php");
...
if(!is_file("$mod_db")) {
	echo '<div class="alert alert-error">No Database found ... we try to create it.</div>';
}
...
if($prefs_version < $mod['version']) {
	...
	$all_tables = glob("../modules/migrate.mod/install/tpl-*.php");
	...
	for($i=0;$i<count($all_tables);$i++) {
		...
		$is_table = mig_table_exists("$db_path","$table_name");
	
		if($is_table < 1) {
			mig_add_table("$db_path","$table_name",$cols);
			$table_updates[] = "<div class='alert alert-info'>New Table: <b>$table_name</b> in Database <b>$database</b></div>";
		}
	
		$existing_cols = mig_get_collumns("$db_path","$table_name");
	
		foreach ($cols as $k => $v) {
			if(!array_key_exists("$k", $existing_cols)) {
				//mig_update_table -> column, type, table, database
				mig_update_table("$k","$cols[$k]","$table_name","$db_path");
...
function mig_get_collumns($db,$table_name) {
...
function mig_table_exists($db,$table_name) {
...
function mig_update_table($col_name,$type,$table_name,$db) {
...
function mig_add_table($db,$table_name,$cols) {
